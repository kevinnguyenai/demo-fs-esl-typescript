...
import {
  api, param, post, requestBody, Response,
  RestBindings
} from '@loopback/rest';
import {FworkServiceBindings} from '../../keys';
import {Click2CallInput, PbxCommandResponse} from '../../models/interfaces/pbx';
import {
  Click2CallInterface, Click2CallService, FworkTranslatorInterfaces
} from '../../services';
...
export class PbxCallController {
  constructor(
    @service(Click2CallService)
    protected fsesl: Click2CallInterface,
            ...
            try {
                ...
                .catch((err: any) => {
                  resolves(
                    response.status(400).send({
                      success: false,
                      msg: `Error in connection to PBX ${err}`,
                      data: [],
                    }),
                  );
                });
              ...
              resolves(
                response.status(500).send({
                  success: false,
                  msg: `Crash with internal service ${err}`,
                  data: [],
                }),
              );
            }
